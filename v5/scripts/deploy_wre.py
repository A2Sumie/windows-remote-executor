"""On-host WRE v5 installer — run with elevation on the Windows target.
...
After deploy, remote control flows through:
    Client -> ssh X570 -> cmd /c "pythonw -I -X utf8 C:/CodexRemote/wre/rpc.py rpc-stdio"
...
def selftest(entry_path: Path, python_exe: Path) -> bool:
    """Run v5 native `selftest` via local python invocation."""
...
def deploy_source(package_wre_dir: Path, dest: Path) -> None:
    ...
    _info(f"v5 tree copied to {dest}")
...
def install_access_policy(dest: Path, listen: str, token: str) -> Path:
    ...
    v5 never writes a null-token policy: a null token fail-closes the bridge
...
def backup_and_rewrite_sshd_config(dest: Path, expected_listen: str, force_command: bool) -> dict:
    ...
    _info("sshd_config updated (ListenAddress"
          + (" + ForceCommand -> WRE v5)" if force_command else ")"))
    return {"rewritten": True, "backup": str(backup_path)}


WRE_FORCE_COMMAND = "pythonw.exe -I -X utf8 C:/CodexRemote/wre/rpc.py rpc-stdio"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--package-wre-dir", default=None,
                        help="Path that contains the v5 source tree. Default: this directory if rpc.py exists, else ./wre inside a bootstrap package")
    ...
    if not args.tasks_only:
        if bool(args.access_token) == bool(args.keep_existing_policy):
            _die("exactly one of --access-token / --keep-existing-policy is required "
                 "(v5 has no null-token default policy)")
    if os.name != "nt":
    ...
    if not wre_entry.is_file():
        _die(f"missing v5 entry: {wre_entry}")
    ...
    _info(f"  PYTHONPATH=. python3 -m v5.scripts.verify_v5_remote {args.target_name}")
    _info("If ssh cannot reach v5, restore the backup sshd_config in")
    _info(f"  {dest / 'backup'}")
    return 0
