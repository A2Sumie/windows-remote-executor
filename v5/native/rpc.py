#!/usr/bin/env python3
"""WRE v5 native rpc-stdio entrypoint.

Runs on the Windows host. Invoked as:
    python.exe -I -X utf8 C:/CodexRemote/wre/rpc.py rpc-stdio

Reads one UTF-8 JSON request line on stdin, writes one UTF-8 JSON
response line on stdout. See V5.md for the protocol contract.

v5 changes vs v4 (see the 2026-08-18 v4 audit):
  - PROTOCOL_VERSION = 5 (capability marker only; wire format unchanged).
  - Fail-closed auth: access-policy.json missing/corrupt/null-token denies
    everything except host.capabilities (see win32/access_policy.py).
  - Best-effort per-request audit log at C:/CodexRemote/logs/rpc-audit.log.
  - Bounded request line (16 MB) instead of an unbounded readline().
"""
...
def _dispatch_inner(request: dict[str, Any], start_perf: float) -> dict[str, Any]:
    ...
    if timeout_seconds is not None:
        # v5 native (like v4) does not enforce the timeout on its own thread;
        # the value is recorded only. The controller enforces the real deadline
        # via its SSH transport timeout. Documented in V5.md.
        pass
...
def dispatch(request: dict[str, Any]) -> dict[str, Any]:
    ...
    response = _dispatch_inner(request, start_perf)
...
def _run_apply_tasks(argv: list[str]) -> int:
    ...
    re-running an elevated command each time v5 changes. Reads a JSON spec file
