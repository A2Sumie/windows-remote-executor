"""WRE v5 access policy enforcement — FAIL-CLOSED.
...
- v5 has NO command-mode argv-only branch — argv is gone entirely.

Fail-closed rules (v5, fixing the v4 fail-open audit findings):
...
# access-policy.json lives at the v5 wre root (next to rpc.py), one level up.
...
def write_policy(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    command_mode = "v5"  # lock to v5; old "argv-only"/"standard" are gone
