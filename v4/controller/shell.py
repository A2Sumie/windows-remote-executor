"""WRE v4 controller shell — single entrypoint.

Usage:
    python3 -m wrev4.shell <target> <action> [payload-json]
    python3 -m wrev4.shell <target> --probe [...]
    python3 -m wrev4.shell <target> --repl
    python3 -m wrev4.shell shell <target>
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="wrev4", description="WRE v4 controller")
...
def _repl(target) -> int:
    print(f"# WRE v4 REPL — target={target.name} ({target.ssh_destination})")
    print("# commands: help, quit, actions, list-targets, <action> <json>")
    while True:
        try:
            line = input(f"wrev4 [{target.name}]> ").strip()
