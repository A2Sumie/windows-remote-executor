"""On-host WRE v4 installer — run with elevation on the Windows target.
...
Layout inside the bootstrap package:
    wre-v0.4.0-windows-x64/
        ...
        wre/                        <- copied to C:/CodexRemote/wre/
            ...
            actions/...
            win32/...
            access-policy.json (initial)
        deploy-wre.py               <- this script

Operator (on the Windows host) elevates to Administrator and runs:
    python\\pythonw.exe deploy-wre.py --target-name NANDTUS --expected-listen 100.111.107.14

After deploy, remote control flows through:
    Client -> ssh NANDTUS -> cmd /c "pythonw -I -X utf8 C:/CodexRemote/wre/rpc.py rpc-stdio"
...
DEFAULT_DEST = Path("C:/CodexRemote/wre")
...
def selftest(entry_path: Path, python_exe: Path) -> bool:
    """Run v4 native `selftest` via local python invocation."""
...
def deploy_source(package_wre_dir: Path, dest: Path) -> None:
    ...
    _info(f"v4 tree copied to {dest}")


def install_initial_access_policy(dest: Path, listen: str, token: str | None) -> Path:
    policy_path = dest / "access-policy.json"
    policy = dict(DEFAULT_ACCESS_POLICY)
    policy["expectedListenAddress"] = listen
    policy["updatedAt"] = datetime.now(timezone.utc).isoformat()
    if token:
        import hashlib
        policy["accessTokenSha256"] = hashlib.sha256(token.encode("utf-8")).hexdigest()
...
def backup_and_rewrite_sshd_config(dest: Path, expected_listen: str) -> dict:
    """Backup `C:/ProgramData/ssh/sshd_config` and rewrite ListenAddress + ForceCommand."""
    ...
    new_text = _patch_sshd_config(original, expected_listen)
    if new_text == original:
        _info("sshd_config already pinned to v4; no change")
        return {"rewritten": False, "backup": str(backup_path), "reason": "already pinned"}
    sshd_mod._atomic_write(str(config_path), new_text)
    _info("sshd_config updated (ListenAddress + ForceCommand -> WRE v4)")
    return {"rewritten": True, "backup": str(backup_path)}


V4_FORCE_COMMAND = "pythonw.exe -I -X utf8 C:/CodexRemote/wre/rpc.py rpc-stdio"
LEGACY_NATIVE_FRAGMENTS = (
    "WindowsRemoteExecutor.Native.exe",
    "WindowsRemoteExecutor.cmd",
    "CodexRemote/tools/",
)
PRE_V4_FORCE_COMMAND_FRAGMENTS = (
    "WRE-v4",  # set by us previously — safe to overwrite
) + LEGACY_NATIVE_FRAGMENTS


def _patch_sshd_config(text: str, expected_listen: str) -> str:
    ...
    for raw in lines:
        ...
        if line and not line.startswith("#"):
            ...
            if key in ("listenaddress", "listen") and not in_match:
                ...
                saw_listen = True
                continue
            if key == "forcecommand" and not in_match:
                if saw_force or any(frag in line for frag in PRE_V4_FORCE_COMMAND_FRAGMENTS) or True:
                    if saw_force:
                        continue
                    out.append(f"ForceCommand {V4_FORCE_COMMAND}")
                    saw_force = True
                    continue
        out.append(raw)
    missing: list[str] = []
    ...
    if not saw_force:
        missing.append(f"ForceCommand {V4_FORCE_COMMAND}")
    if missing:
        insert_at = None
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--package-wre-dir", default=None,
                        help="Path that contains the v4 source tree. Default: this directory if rpc.py exists, else ./wre inside a bootstrap package")
    parser.add_argument("--dest", default=str(DEFAULT_DEST),
                        help="Destination directory on the host (default C:/CodexRemote/wre)")
    ...
    parser.add_argument("--register-streamserv", action="store_true",
                        help="Also register CodexRemote StreamServ Start (TaskScheduler COM) for X570")
    ...
    if not wre_entry.is_file():
        _die(f"missing v4 entry: {wre_entry}")
    ...
    install_initial_access_policy(dest, args.expected_listen, args.access_token)
    ...
    _info("step: backup + rewrite sshd_config (ListenAddress + ForceCommand)")
    sshd_state = backup_and_rewrite_sshd_config(dest, args.expected_listen)
    ...
    _info(f"  python3 -m v4.scripts.verify_v4_remote {args.target_name}")
    _info("If ssh cannot reach v4, restore the backup sshd_config in")
    _info(f"  {dest / 'backup'}")
    return 0
