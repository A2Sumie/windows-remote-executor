...
from version import TASK_PREFIX, WRE_TREE  # noqa: E402
...
def list_tasks(*, prefix: str = TASK_PREFIX, names: list[str] | None = None) -> list[dict[str, Any]]:
    try:
...
# ---------- tasks.clean filtering (pure function — loopback-unit-tested) ----------

_NEVER_RUN_SENTINEL_YEAR = 1899  # Task Scheduler's LastRunTime for never-run tasks


def parse_task_time(value: Any) -> datetime | None:
    """Parse a task LastRunTime/NextRunTime into an aware datetime; None means
    never-run (empty or the 1899-12-30 COM sentinel)."""
    if not value:
        return None
    try:
        ...
        return None
    if dt.year <= _NEVER_RUN_SENTINEL_YEAR + 1:
        return None
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)  # coarse day-granularity compare
    return dt


def filter_tasks_for_clean(
    tasks: list[dict[str, Any]],
    *,
    prefix: str,
    older_than_days: int,
    include_never_run: bool = False,
    keep_names: tuple[str, ...] = (),
    now: datetime | None = None,
) -> list[dict[str, Any]]:
    """Return tasks that match ALL of: name starts with prefix, not in
    keep_names, last run older than older_than_days (never-run tasks only
    when include_never_run). Pure: unit-tested without TaskScheduler."""
    ...
    for task in tasks:
        ...
        if prefix and not name.startswith(prefix):
            continue
        last = parse_task_time(task.get("lastRunTime"))
        if last is None:
            if not include_never_run:
                continue
        elif last >= cutoff:
            continue
        out.append(task)
...
def create_task(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    `spec`:
      ...
      delete_after_run / deleteAfterRun (bool) — self-delete at expiresAt:
        attaches a DISABLED time trigger carrying EndBoundary (never fires on
        its own; host.task.run still works) and sets
        Settings.DeleteExpiredTaskAfter = PT0S. Default expiry: +24h.
      expiresAt (ISO-8601) — explicit expiry instant for deleteAfterRun.
    """
    ...
    delete_after_run = bool(spec.get("deleteAfterRun") or spec.get("delete_after_run"))
    expires_at = spec.get("expiresAt")
    ...
    expiry_note = ""
    if delete_after_run:
        expiry_note = _set_delete_after_run(definition, expires_at)
...
def _set_delete_after_run(definition, expires_at: Any) -> str:  # type: ignore[no-untyped-def]
    """Make the task self-delete at expiry.

    Task Scheduler deletes a task when its last trigger's EndBoundary passes
    AND Settings.DeleteExpiredTaskAfter is set. A manual task has no trigger,
    so we attach a DISABLED time trigger purely as the boundary carrier — it
    never fires on its own (host.task.run still triggers the task manually).
    """
    if expires_at:
        try:
        ...
        expiry = datetime.now() + timedelta(hours=24)
    boundary = expiry.replace(microsecond=0).isoformat()
    start = (expiry - timedelta(minutes=1)).replace(microsecond=0).isoformat()
    trigger = definition.Triggers.Create(TASK_TRIGGER_TIME)
    trigger.StartBoundary = start
    trigger.EndBoundary = boundary
    trigger.Enabled = False
    definition.Settings.DeleteExpiredTaskAfter = "PT0S"
    return f"auto-delete at {boundary} (disabled boundary trigger; run manually any time before)"
...
def ensure_repair_tasks(*, expected_listen: str | None) -> dict[str, Any]:
    """Re-create the three `<TASK_PREFIX> Sshd Repair` tasks, pointing at the
    RUNNING tree (WRE_TREE) — a sidecar wre6 repairs with its own rpc.py."""
    pythonw = f"{WRE_TREE}/python/pythonw.exe"
    wre_entry = f"{WRE_TREE}/rpc.py"

    task_defs: list[dict[str, Any]] = [
        {"name": f"{TASK_PREFIX} Sshd Repair Logon", "trigger": "logon", "delay_iso": "PT30S"},
        {"name": f"{TASK_PREFIX} Sshd Repair Startup", "trigger": "startup", "delay_iso": "PT60S"},
        {"name": f"{TASK_PREFIX} Sshd Repair Watch", "trigger": "boot",
    ...
    for spec in task_defs:
        try:
            create_task({
                ...
                "description": "WRE v6 sshd self-repair",
            })
...
def ensure_streamserv_task(*, root: str = "D:/StreamServ", run_as_user: str = "SYSTEM") -> dict[str, Any]:
    ...
    root = root.rstrip("/\\")
    return create_task({
        "name": f"{TASK_PREFIX} StreamServ Start",
...
def ensure_apply_agent_task(*, expected_listen: str | None = None) -> dict[str, Any]:
    """Register the SYSTEM task-registration agent `<TASK_PREFIX> WRE Apply`.

    Once this SYSTEM task exists, the controller can re-register every managed
    task by triggering it (`host.task.run {name:"<TASK_PREFIX> WRE Apply"}`) —
    no more elevated operator command each time v6 changes. This task itself
    ...
    return create_task({
        "name": f"{TASK_PREFIX} WRE Apply",
        "exe": pythonw,
        "args": args,
        "trigger": "manual",
        "run_as_user": "SYSTEM",
        "run_level": TASK_RUNLEVEL_HIGHEST,
        "description": "WRE v6 SYSTEM task-registration agent (re-applies managed tasks on demand)",
    })
