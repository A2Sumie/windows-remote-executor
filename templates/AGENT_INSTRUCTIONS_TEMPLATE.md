...
4. For locked-down hosts, set `win-remote policy --command-mode argv-only`; then use `run`/`capture` with concrete native executables and explicit argv, or `exec` only for explicit script maintenance. Do not use `py`, `wsl*`, or shell/interpreter executables through `run`/`capture` there.
