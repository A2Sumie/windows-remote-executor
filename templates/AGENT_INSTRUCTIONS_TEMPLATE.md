...
8. Do not send raw `powershell.exe`, `pwsh`, or hand-rolled `-EncodedCommand` over SSH.
9. Assume the host should remain `private-only` unless the operator explicitly says otherwise.
10. Never weaken `access-policy.json`, remove the `sshd` guard, or permit wildcard listeners without explicit operator approval.
11. Never commit real host env files, access tokens, hostnames, Tailscale IPs, usernames, SSH keys, logs, or publish outputs.
12. Prefer the framework-dependent `.NET 8` publish when the Windows host already has the `.NET 8` runtime installed.
13. Use the self-contained publish only when the host cannot satisfy the framework-dependent runtime requirement.
